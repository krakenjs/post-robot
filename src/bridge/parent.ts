...
export function listenForOpenTunnel({
    ...
    on(MESSAGE_NAME.OPEN_TUNNEL, ({ source, origin, data }) => {
        ...
        // @ts-ignore need to supply generic for what `bridge` is
...
export function openBridge(
    ...
    const bridges = globalStore<ZalgoPromise<any>>('bridges');
                ...
                return new ZalgoPromise((resolve, reject) => {
                    ...
                    .then(() => {
                        return awaitWindowHello(
                            // @ts-ignore - bridge could be null
                            bridge,
                            BRIDGE_TIMEOUT,
                            `Bridge ${ url }`
                        );
...
export function linkWindow({ win, name, domain }: WinDetails): WinDetails {
    ...
    for (const winName of popupWindowsByName.keys()) {
        ...
        // @ts-ignore this is unknown type
...
export function linkUrl(win: CrossDomainWindowType, url: string): void {
    linkWindow({
        win,
        domain: getDomainFromUrl(url)
    });
}
export function listenForWindowOpen(): void {
    const windowOpen = window.open;

    // @ts-ignore - reassign window.open
...
export function destroyBridges(): void {
    const bridges = globalStore('bridges');
    const bridgeFrames = globalStore('bridgeFrames');
