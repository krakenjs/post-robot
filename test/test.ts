...
import { uniqueID, noop } from 'belter';
...
function getBody(): HTMLBodyElement | HTMLElement {
...
function createIframe(
    ...
    return frame.contentWindow as CrossDomainWindowType;
}

function createPopup(name: string): CrossDomainWindowType {
    const popup = window.open(
        `mock://test-post-robot-child.com/base/test/${ name }`,
        `${ Math.random().toString() }_${ name.replace(/[^a-zA-Z0-9]+/g, '_') }`
    );
    window.focus();
    return popup as CrossDomainWindowType;
}

let childWindow: CrossDomainWindowType;
let childFrame: CrossDomainWindowType;
let otherChildFrame: CrossDomainWindowType;

// declaring test globals since this isnt a *.tests.ts file and appears to use nonstandard things
declare function after(callback: any): any;
declare function before(callback: any): any;
declare function it(message: string, callback: any): any;
...
after(() => {
    ...
    if (!otherChildFrame?.frameElement) {
        throw new Error(
            `Expected otherChildFrame.frameElement to be available`
        );
    }

    body.removeChild(otherChildFrame.frameElement);
    childWindow?.close(); // eslint-disable-line no-unused-expressions
});
describe('[post-robot] happy cases', () => {
    it('should set up a simple server and listen for a request', (done: any) => {
        on('foobu', () => {
            done();
        });
        // @ts-ignore
    ...
    it('should message a child and expect a response', (): ZalgoPromise<unknown> => {
        return send(childFrame, 'setupListener', {
            ...
            return send(childFrame, 'foo').then(({ data }: any) => {
                if (data.foo !== 'bar') {
                    throw new Error(
                        `Expected data.foo to be 'bar', got ${ data.foo }`
                    );
                }
            });
        });
    });
    it('should set up a simple server and listen for a request from a specific domain', (done: any) => {
        on(
    ...
    it('should message a child with a specific domain and expect a response', (): ZalgoPromise<unknown> => {
        return send(
            ...
            return send(childFrame, 'domainspecificmessage').then(
                ({ data }: any) => {
                    if (data.foo !== 'bar') {
                        throw new Error(
                            `Expected data.foo to be 'bar', got ${ data.foo }`
    ...
    it('should set up a simple server and listen for a request from multiple domains', (done: any) => {
        on(
    ...
    it('should message a child with multiple domains and expect a response', (): ZalgoPromise<unknown> => {
        return send(
            ...
            return send(childFrame, 'multidomainspecificmessage').then(
                ({ data }: any) => {
                    if (data.foo !== 'bar') {
                        throw new Error(
                            `Expected data.foo to be 'bar', got ${ data.foo }`
    ...
    it('should be able to listen for a message only once', (): ZalgoPromise<unknown> => {
        ...
        return send(childFrame, 'sendMessageToParent', {
            ...
            return send(childFrame, 'sendMessageToParent', {
                ...
                () => {
                    throw new Error(
                        'Expected success handler to not be called'
                    );
                },
                () => {
                    if (count !== 1) {
...
describe('[post-robot] serialization cases', () => {
    it('should pass a function across windows and be able to call it later', (done: any) => {
        const expectedArgument = 567;
        let actualArgument: any;
    ...
    it('should pass a function across windows and be able to call it later and capture the exception', (done: any) => {
        const expectedErrorMessage = 'something went wrong';
        const expectedErrorCode = 'ERROR_567';
        let expectedErrorStack: any;
    ...
    it('should pass a function across windows and be able to call it instantly from its origin window', (done: any) => {
        const expectedArgument = 567;
        let actualArgument: any;
        const expectedReturn = 'hello world';
    ...
    it('should pass a promise across windows and be able to call it later', (done: any) => {
    ...
    it('should pass a promise across windows and be able to reject it later', (done: any) => {
        const expectedErrorMessage = 'Oh no!';
        const expectedErrorCode = 'ABC123';
        let expectedErrorStack: any; // eslint-disable-line prefer-const
    ...
    it('should pass a zalgo promise across windows and be able to call it later', (done: any) => {
        const expectedValue = 123;
        let resolver;
    ...
    it('should pass a zalgo promise across windows and be able to reject it later', (done: any) => {
        const expectedErrorMessage = 'Oh no!';
        const expectedErrorCode = 'ABC123';
        let expectedErrorStack: any; // eslint-disable-line prefer-const
    ...
    it('should pass an iframe across the window boundary and change its location', () => {
        const iframe = document.createElement('iframe');
        getBody().appendChild(iframe);
        const mywindow = iframe.contentWindow as CrossDomainWindowType;
        return send(childFrame, 'setupListener', {
            messageName:'foo',
            data:       {
                mywindow
            ...
            .then(({ data }: any) => {
                if (data.hello !== 'world') {
                    throw new Error(
                        `Expected hello to equal world, got ${ data.hello }`
                    );
                }
            });
    });
    ...
    it('should get window type for popup even if window closed after serialization', () => {
        ...
        return send(childFrame, 'setupListener', {
            ...
            .then(({ data }: any) => {
                mywindow?.close(); // eslint-disable-line no-unused-expressions
    ...
    it('should get window type for iframe even if window closed after serialization', () => {
        ...
        return send(childFrame, 'setupListener', {
            ...
            .then(({ data }: any) => {
                // @ts-ignore
                mywindow.frameElement.parentNode.removeChild(
                    mywindow.frameElement as Element
                );
                return data.mywindow.getType();
            })
            .then((type) => {
    ...
    it('should error getting window type for popup if window is closed prior to serialization', () => {
        ...
        return send(childFrame, 'setupListener', {
            ...
            .then(({ data }: any) => {
                return data.mywindow.getType().catch((err: any) => {
                    error = err;
                });
            })
            .then(() => {
...
describe('[post-robot] error cases', () => {
    ...
    it('should fail to send a message when the expected domain does not match', (done: any) => {
        on(
            'foobuzzzzz',
            {
        ...
        send(childFrame, 'sendMessageToParent', {
            ...
            () => {
                return done(
                    new Error('Expected success handler to not be called')
                );
            },
            (err) => {
                if (!err) {
...
describe('[post-robot] popup tests', () => {
    it('should work with a popup window', (): ZalgoPromise<unknown> => {
        return send(childWindow, 'setupListener', {
            ...
            return send(childWindow, 'foo').then(({ data }: any) => {
                if (data.foo !== 'bar') {
                    throw new Error(
                        `Expected data.foo to be 'bar', got ${ data.foo }`
                    );
