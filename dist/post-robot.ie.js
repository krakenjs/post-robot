!function(root, factory) {
    ...
    return function(modules) {
        ...
        function needsBridgeForBrowser() {
            return !!Object(cross_domain_utils.getUserAgent)(window).match(/MSIE|trident|edge\/12|edge\/13/i);
        }
        function needsBridgeForWin(win) {
            return !Object(cross_domain_utils.isSameTopWindow)(window, win);
        }
        function needsBridgeForDomain(domain, win) {
            if (domain) {
                if (Object(cross_domain_utils.getDomain)() !== Object(cross_domain_utils.getDomainFromUrl)(domain)) return !0;
            } else if (win && !Object(cross_domain_utils.isSameDomain)(win)) return !0;
            return !1;
        }
        ...
        function getBridgeName(domain) {
            return "__postrobot_bridge___" + (domain = domain || Object(cross_domain_utils.getDomainFromUrl)(domain)).replace(/[^a-zA-Z0-9]+/g, "_");
        }
        function isBridge() {
            return Boolean(window.name && window.name === getBridgeName(Object(cross_domain_utils.getDomain)()));
        }
        var documentBodyReady = new zalgo_promise.ZalgoPromise((function(resolve) {
        ...
        function registerRemoteSendMessage(win, domain, sendMessage) {
            findRemoteWindow(win).resolve((function(remoteWin, remoteDomain, message) {
                ...
                if (!Object(cross_domain_utils.matchDomain)(remoteDomain, domain)) throw new Error("Remote domain " + remoteDomain + " does not match domain " + domain);
                sendMessage.fireAndForget(message);
            }));
        }
        function rejectRemoteSendMessage(win, err) {
            findRemoteWindow(win).reject(err).catch(esm_util_noop);
        }
        function linkWindow(_ref3) {
            ...
            for (var _i2 = 0, _popupWindowsByName$k2 = popupWindowsByName.keys(); _i2 < _popupWindowsByName$k2.length; _i2++) {
                ...
                _details && !Object(cross_domain_utils.isWindowClosed)(_details.win) || popupWindowsByName.del(winName);
            }
            if (Object(cross_domain_utils.isWindowClosed)(win)) return {
                win: win,
                name: name,
                domain: domain
            };
        ...
        function setupBridge(_ref) {
            ...
            windowOpen = window.open, window.open = function(url, name, options, last) {
                var win = windowOpen.call(this, Object(cross_domain_utils.normalizeMockUrl)(url), name, options, last);
                if (!win) return win;
                linkWindow({
                    win: win,
                    name: name,
                    domain: url ? Object(cross_domain_utils.getDomainFromUrl)(url) : null
                });
                return win;
            };
            var windowOpen;
                            ...
                            sendMessage: function(message) {
                                if (window && !window.closed && getWindowDetails()) {
                                    ...
                                    if (domain) try {
                                        ...
                                        zalgo_promise.ZalgoPromise.reject(err);
                                    }
                                }
                            }
                        };
            ...
            !function(_ref2) {
                ...
                global_getGlobal(window).openTunnelToParent = function(_ref3) {
                    ...
                    var parentWindow = Object(cross_domain_utils.getParent)(window);
                    ...
                    var id = function(_ref) {
                        ...
                        !function() {
                            ...
                            for (var _i2 = 0, _tunnelWindows$keys2 = tunnelWindows.keys(); _i2 < _tunnelWindows$keys2.length; _i2++) {
                                ...
                                Object(cross_domain_utils.isWindowClosed)(tunnelWindow.source) && tunnelWindows.del(key);
                            }
                        }();
                        var id = uniqueID();
                    ...
                    return send(parentWindow, "postrobot_open_tunnel", {
                        ...
                        sendMessage: function() {
                            ...
                            if (tunnelWindow && tunnelWindow.source && !Object(cross_domain_utils.isWindowClosed)(tunnelWindow.source)) {
                                try {
                                    tunnelWindow.canary();
                                } catch (err) {
                                    return;
            ...
            !function(_ref) {
                ...
                zalgo_promise.ZalgoPromise.try((function() {
                    var opener = Object(cross_domain_utils.getOpener)(window);
                    if (opener && needsBridge({
                        ...
                        return (win = opener, windowStore("remoteBridgeAwaiters").getOrSet(win, (function() {
                            return zalgo_promise.ZalgoPromise.try((function() {
                                var frame = Object(cross_domain_utils.getFrameByName)(win, getBridgeName(Object(cross_domain_utils.getDomain)()));
                                if (frame) return Object(cross_domain_utils.isSameDomain)(frame) && global_getGlobal(Object(cross_domain_utils.assertSameDomain)(frame)) ? frame : new zalgo_promise.ZalgoPromise((function(resolve) {
                                    var interval;
                                    var timeout;
                                    interval = setInterval((function() {
                                        if (frame && Object(cross_domain_utils.isSameDomain)(frame) && global_getGlobal(Object(cross_domain_utils.assertSameDomain)(frame))) {
                                            clearInterval(interval);
                            ...
                            if (bridge) {
                                if (window.name) return global_getGlobal(Object(cross_domain_utils.assertSameDomain)(bridge)).openTunnelToParent({
                                    name: window.name,
                                    source: window,
                                    canary: function() {},
                                    sendMessage: function(message) {
                            ...
                            } else rejectRemoteSendMessage(opener, new Error("Can not register with opener: no bridge found in opener"));
                        }));
                        var win;
                    }
                }));
        ...
        SEND_MESSAGE_STRATEGIES.postrobot_bridge = function(win, serializedMessage, domain) {
            ...
            if (Object(cross_domain_utils.isSameDomain)(win)) throw new Error("Post message through bridge disabled between same domain windows");
            if (!1 !== Object(cross_domain_utils.isSameTopWindow)(window, win)) throw new Error("Can only use bridge to communicate between two different windows, not between frames");
            !function(win, domain, message) {
                var messagingChild = Object(cross_domain_utils.isOpener)(window, win);
                var messagingParent = Object(cross_domain_utils.isOpener)(win, window);
        ...
        SEND_MESSAGE_STRATEGIES.postrobot_global = function(win, serializedMessage) {
            if (!Object(cross_domain_utils.getUserAgent)(window).match(/MSIE|rv:11|trident|edge\/12|edge\/13/i)) throw new Error("Global messaging not needed for browser");
            if (!Object(cross_domain_utils.isSameDomain)(win)) throw new Error("Post message through global disabled between different domain windows");
            if (!1 !== Object(cross_domain_utils.isSameTopWindow)(window, win)) throw new Error("Can only use global to communicate between two different windows, not between frames");
        ...
        var send_send = function send(winOrProxyWin, name, data, options) {
            ...
            return window_ProxyWindow.toProxyWindow(winOrProxyWin, {
                ...
                return zalgo_promise.ZalgoPromise.try((function() {
                    ...
                    if (Object(cross_domain_utils.isAncestor)(window, win)) return awaitWindowHello(win, childTimeout);
                })).then((function(_temp) {
        ...
        src_bridge = {
            ...
            openBridge: function(url, domain) {
                ...
                domain = domain || Object(cross_domain_utils.getDomainFromUrl)(url);
                return bridges.getOrSet(domain, (function() {
                    return zalgo_promise.ZalgoPromise.try((function() {
                        if (Object(cross_domain_utils.getDomain)() === domain) throw new Error("Can not open bridge on the same domain as current domain: " + domain);
                        var name = getBridgeName(domain);
                        if (Object(cross_domain_utils.getFrameByName)(window, name)) throw new Error("Frame with name " + name + " already exists on page");
            ...
            linkUrl: function(win, url) {
                linkWindow({
                    win: win,
                    domain: Object(cross_domain_utils.getDomainFromUrl)(url)
                });
            },
            isBridge: isBridge,
            ...
            hasBridge: function(url, domain) {
                return globalStore("bridges").has(domain || Object(cross_domain_utils.getDomainFromUrl)(url));
            },
