!function(root, factory) {
    ...
    return function(modules) {
        ...
        function utils_isOpener(parent, child) {
            return parent === utils_getOpener(child);
        }
        ...
        function needsBridgeForBrowser() {
            return !!(win = window, (win = win || window).navigator.mockUserAgent || win.navigator.userAgent).match(/MSIE|trident|edge\/12|edge\/13/i);
            var win;
        }
        function needsBridgeForWin(win) {
            return !function(win1, win2) {
        ...
        function needsBridgeForDomain(domain, win) {
            if (domain) {
                if (utils_getDomain() !== utils_getDomainFromUrl(domain)) return !0;
            } else if (win && !function(win) {
                if (!function(win) {
                    try {
                        if (win === window) return !0;
                    } catch (err) {}
        ...
        function getBridgeName(domain) {
            return "__postrobot_bridge___" + (domain = domain || utils_getDomainFromUrl(domain)).replace(/[^a-zA-Z0-9]+/g, "_");
        }
        function isBridge() {
        ...
        function registerRemoteSendMessage(win, domain, sendMessage) {
            findRemoteWindow(win).resolve((function(remoteWin, remoteDomain, message) {
                ...
                if (!function utils_matchDomain(pattern, origin) {
                    if ("string" == typeof pattern) {
                        if ("string" == typeof origin) return "*" === pattern || origin === pattern;
                        if (util_isRegex(origin)) return !1;
                        if (Array.isArray(origin)) return !1;
                    }
                    return util_isRegex(pattern) ? util_isRegex(origin) ? pattern.toString() === origin.toString() : !Array.isArray(origin) && Boolean(origin.match(pattern)) : !!Array.isArray(pattern) && (Array.isArray(origin) ? JSON.stringify(pattern) === JSON.stringify(origin) : !util_isRegex(origin) && pattern.some((function(subpattern) {
                        return utils_matchDomain(subpattern, origin);
                    })));
                }(remoteDomain, domain)) throw new Error("Remote domain " + remoteDomain + " does not match domain " + domain);
                sendMessage.fireAndForget(message);
            }));
        }
        function rejectRemoteSendMessage(win, err) {
            findRemoteWindow(win).reject(err).catch(belter_src_util_noop);
        }
                ...
                getGlobal(window).openTunnelToParent = function(_ref3) {
                    ...
                    var id = function(_ref) {
                        ...
                        !function() {
                            ...
                            for (var _i2 = 0, _tunnelWindows$keys2 = tunnelWindows.keys(); _i2 < _tunnelWindows$keys2.length; _i2++) {
                                ...
                                try {
                                    Object(belter.noop)(tunnelWindow.source);
                                } catch (err) {
                                    tunnelWindows.del(key);
                                    continue;
                                }
                                isWindowClosed(tunnelWindow.source) && tunnelWindows.del(key);
                            }
                        }();
                        var id = Object(belter.uniqueID)();
                    ...
                    return send(parentWindow, "postrobot_open_tunnel", {
                        ...
                        sendMessage: function() {
                            ...
                            try {
                                Object(belter.noop)(tunnelWindow && tunnelWindow.source);
                            } catch (err) {
                    ...
                    if (opener && needsBridge({
                        ...
                        return (win = opener, windowStore("remoteBridgeAwaiters").getOrSet(win, (function() {
                            return promise_ZalgoPromise.try((function() {
                                ...
                                if (frame) return isSameDomain(frame) && getGlobal(assertSameDomain(frame)) ? frame : new promise_ZalgoPromise((function(resolve) {
                                    var interval;
                                    var timeout;
                            ...
                            return bridge ? window.name ? getGlobal(assertSameDomain(bridge)).openTunnelToParent({
                                name: window.name,
                                source: window,
                                canary: function() {},
                                sendMessage: function(message) {
                                    try {
                                        Object(belter.noop)(window);
                                    } catch (err) {
                                        return;
                                    }
        ...
        SEND_MESSAGE_STRATEGIES.postrobot_bridge = function(win, serializedMessage, domain) {
            ...
            !function(win, domain, message) {
                var messagingChild = utils_isOpener(window, win);
                var messagingParent = utils_isOpener(win, window);
        ...
        SEND_MESSAGE_STRATEGIES.postrobot_global = function(win, serializedMessage) {
            if (!function() {
                return !!(win = window, (win = win || window).navigator.mockUserAgent || win.navigator.userAgent).match(/MSIE|rv:11|trident|edge\/12|edge\/13/i);
                var win;
